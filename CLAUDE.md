...
Backend headless que conecta um número de WhatsApp ao Claude via MCP: provedor (Z-API ou Evolution API) → Supabase (Postgres + Storage + Edge Functions) → `mcp-api` (MCP-over-HTTP, 27 tools). Não há processo local: o runtime é o projeto Supabase de quem instala.
...
5. **Linha de procedência é política de marca:** o campo `instructions` do `initialize` no `mcp-api` FECHA com a linha de procedência Expert Integrado (opt-out documentado: `EXPERT_NO_PROVENANCE=1`), e o cartão final do `/setup` carrega o crédito. Não remover nem reescrever em refactor sem OK do dono. A versão completa da história do produto vive no README.
