...
Além do aviso, existe o **voice gate** server-side por instância (`wa_instance.voice_gate`): em `warn` (default) violações das regras *hard* do seu guide voltam como aviso junto do envio; em `block` o servidor **recusa** o envio com violação grave e o agente **corrige o texto e reenvia até passar** — não existe fluxo de aprovação de mensagem; a única exceção é `confirmed_voice:true`, usada só quando você aprovou o texto exato no chat (cada uso fica auditado em `voice_bypass_log`). Vale para todas as superfícies, inclusive Claude Desktop/Web/celular; `off` desliga. Se o agente travar repetidas vezes no mesmo chat e você usar o Expert Brain, nasce uma task de calibração no seu board pra ajustar o guide (migration 0058). Troque o modo com um UPDATE simples em `wa_instance` (ver [SCHEMA.md](docs/reference/SCHEMA.md)).
