...
// vault via env var OP_SECRETS_VAULT (default: "WhatsApp Agent").
//
...
const OP_VAULT = process.env.OP_SECRETS_VAULT || "WhatsApp Agent";
...
const OP_REFS = {
  SUPABASE_URL:   `op://${OP_VAULT}/SUPABASE_URL/credential`,
  MCP_API_KEY:    `op://${OP_VAULT}/MCP_API_KEY/credential`,
...
// Regras hard universais — fingerprints binarias de "nao-dono" (e fingerprints de IA em geral)
...
const HARD_RULES = [
  {
    ...
    severity: "high",
    message: "Detectado uso de 'tu/teu/tua' — voice guide proibe em qualquer estrato. o dono usa 'vc'/'seu'.",
  },
  {
    id: "em-dash",
    pattern: /—/,
  ...
  },
  {
    ...
    severity: "medium",
    message: "Detectado softener. Em equipe o dono usa ordem direta ('tem q resolver X'). Em discordancia, frontalidade direta.",
  },
  {
...
- waiting_on: "me" (lead respondeu por ultimo, eu devo responder), "lead" (eu respondi por ultimo, espera deles), "none"
...
Mensagens de audio incluem campo transcription transcrito automaticamente.`,
  {
    ...
    waiting_on: z.enum(["me", "lead", "none"]).optional().describe("Filtra por quem deve responder agora"),
...
2a chamada — COM confirmed: true: so apos o usuario confirmar explicitamente ("sim", "confirma", "pode enviar").`,
  {
    ...
    force_send_after_inbound: z.boolean().default(false).describe("Se true, ignora o gate de inbound recente nao respondido. Default false: se a pessoa enviou algo nos ultimos 10 minutos e voce ainda nao respondeu, o send eh bloqueado pra evitar perder contexto. Use true APOS confirmar com o dono que ele quer enviar mesmo assim."),
    ...
    try {
      ...
      if (type === "text" && content && !r.blocked && !r.ambiguous) {
        ...
        if (violations.length > 0) {
          ...
          voiceWarning = { voice_guide_check: "passed", note: "Mensagem compativel com voice guide." };
        }
      }
...
//
// Skill `voz` (catalog de perfis de voz)
...
QUANDO USAR:
- o dono pediu audio explicitamente ("manda audio pro X", "responde em audio", "manda mensagem de voz")
- Skill 'voz' resolveu qual voice_id + settings usar baseado no perfil de voz configurado
...
Slugs sao normalizados (lowercase ascii). o dono pode adicionar categorias novas
...
- "manual" — o dono atribuiu (default)
...
de saude que o dono paga). Slug unico por chat (PK chat_id+category_id).
...
Funciona apenas em mensagens de texto enviadas por voce (from_me=true).`,
  ...
  async ({ message_id, new_content, confirmed }) => {
    ...
    try {
      const r = await callApi("edit_message", { message_id, new_content });
      return r.error ? err(r.error) : ok(r);
    } catch (e) { return err(e.message); }
  }
);
...
Precisa do message_id (UUID da tabela messages) — obtenha via read ou search.`,
  ...
  async ({ message_id, confirmed }) => {
    ...
    try {
      const r = await callApi("delete_message", { message_id });
      return r.error ? err(r.error) : ok(r);
    } catch (e) { return err(e.message); }
  }
);
