...
function callWaProxy(action: string, wparams: Record<string, unknown>, instance: string | null) {
  return callEdge("wa-proxy", { action, params: wparams, confirmed: true, agent_name: "mcp-api", agent_request_id: crypto.randomUUID(), instance });
}

async function defaultInstanceId(instance?: string): Promise<string | null> {
  if (instance) return await resolveInstanceKey(instance);
  const rows = await loadInstances();
  return rows.find((i: any) => i.is_default)?.instance_id ?? rows[0]?.instance_id ?? null;
}

// resolveGroup: resolveChat + exige grupo; devolve tambem provider e instancia efetiva.
// Aceita nome do grupo, chat_id ("...@g.us"/"...-group") ou digitos.
async function resolveGroup(group: string, instance?: string): Promise<any> {
  const resolved = await resolveChat(group, instance);
  if (resolved.error || resolved.candidates) return resolved;
  let meta: any = null;
  if (resolved.instance_id) {
    const { data } = await supabase.from("chats").select("is_group,member_count")
      .eq("chat_id", resolved.chat_id).eq("instance_id", resolved.instance_id).limit(1);
    meta = data?.[0] ?? null;
  }
  // Sem linha no banco, so aceita id que JA tem cara de grupo (grupo novo ainda nao sincado).
  const looksLikeGroupId = /-group$|@g\.us$/.test(String(resolved.chat_id));
  if (meta ? !meta.is_group : !looksLikeGroupId) {
    return { error: `"${group}" resolveu pra um chat que nao e grupo (${resolved.chat_id}). Passe o nome do grupo ou um id "...@g.us"/"...-group".` };
  }
  const instId = resolved.instance_id ?? await defaultInstanceId(instance);
  if (!instId) return { error: "Nenhuma instancia configurada." };
  const provider = (await loadInstances()).find((i: any) => i.instance_id === instId)?.provider ?? "zapi";
  // Grupo sem linha no banco: normaliza o id digitado pro dialeto de STORAGE do provider
  // (evolution grava "@g.us", zapi grava "-group"). Sem isso, group_info com o id no
  // dialeto "errado" criaria um chat fantasma duplicado no sync.
  let chatId = resolved.chat_id;
  if (!meta) {
    const digits = String(chatId).split("@")[0].replace(/\D/g, "");
    if (digits) chatId = provider === "evolution" ? `${digits}@g.us` : `${digits}-group`;
  }
  return { ...resolved, chat_id: chatId, instance_id: instId, provider, ...(meta?.member_count != null && { member_count: meta.member_count }) };
}

// Metadata cru do provider -> shape neutro (Z-API group-metadata / Evolution findGroupInfos).
function normalizeGroupMetadata(provider: string, raw: any): any | null {
  if (!raw || typeof raw !== "object") return null;
  if (provider === "evolution") {
    const parts = (Array.isArray(raw.participants) ? raw.participants : []).map((p: any) => ({
      phone: String(p.id ?? "").split("@")[0].replace(/\D/g, ""),
      is_admin: p.admin === "admin" || p.admin === "superadmin",
      is_super_admin: p.admin === "superadmin",
    })).filter((p: any) => p.phone);
    return {
      group_id: raw.id ?? null,
      name: raw.subject ?? null,
      description: raw.desc ?? null,
      owner: raw.owner ? String(raw.owner).split("@")[0].replace(/\D/g, "") : null,
      invite_link: raw.inviteUrl ?? null,
      settings: { admin_only_message: !!raw.announce, admin_only_settings: !!raw.restrict },
      participants: parts,
      participant_count: raw.size ?? parts.length,
    };
  }
  const parts = (Array.isArray(raw.participants) ? raw.participants : []).map((p: any) => ({
  ...
  return {
    group_id: raw.phone ?? null,
    name: raw.subject ?? null,
    description: raw.description ?? null,
    owner: raw.owner ?? null,
    invite_link: raw.invitationLink ?? null,
    settings: {
      admin_only_message: !!raw.adminOnlyMessage,
      admin_only_settings: !!raw.adminOnlySettings,
      require_admin_approval: !!raw.requireAdminApproval,
      admin_only_add_member: !!raw.adminOnlyAddMember,
    },
    participants: parts,
...
// Espelha metadata normalizado em chats + group_participants (sync profundo por grupo).
async function syncGroupToDb(instanceId: string | null, chatId: string, meta: any): Promise<void> {
  if (!instanceId || !meta) return;
  await supabase.from("chats").upsert({
    instance_id: instanceId, chat_id: chatId, is_group: true,
    ...(meta.name && { chat_name: meta.name }),
    ...(meta.description !== undefined && { description: meta.description }),
    ...(meta.participant_count != null && { member_count: meta.participant_count }),
  }, { onConflict: "instance_id,chat_id" });
  const phones: string[] = meta.participants.map((p: any) => p.phone);
  if (!phones.length) return;
  await supabase.from("group_participants").upsert(meta.participants.map((p: any) => ({
    instance_id: instanceId, chat_id: chatId, phone: p.phone,
    is_admin: p.is_admin, is_super_admin: p.is_super_admin, left_at: null,
    ...(p.name && { name: p.name }),
  })), { onConflict: "instance_id,chat_id,phone" });
  // Quem esta no banco mas nao veio no metadata saiu do grupo.
  // ponytail: acima de 300 participantes o filtro in() estoura a URL do PostgREST —
  // pula a marcacao (o webhook group_participant cobre saidas); chunking se precisar.
  if (phones.length <= 300) {
    await supabase.from("group_participants").update({ left_at: new Date().toISOString() })
      .eq("instance_id", instanceId).eq("chat_id", chatId).is("left_at", null)
...
async function dispatchAction(action: string, params: any = {}): Promise<Response> {
  try {
    switch (action) {
      ...
      case "group_create": {
        const { name, participants = [], description, auto_invite = true, instance, confirmed = false } = params;
        if (!name || !Array.isArray(participants) || !participants.length) return json({ error: "name e participants (min 1 telefone) sao obrigatorios." }, 400);
        if (!confirmed) return json({ blocked: true, needs_confirmation: true, name, participants, ...(description && { description }), instruction: "Criar grupo adiciona/convida os participantes na hora. Mostre nome + participantes ao usuario e reenvie com confirmed:true apos ele confirmar." });
        const targetInst = await defaultInstanceId(instance);
        if (!targetInst) return json({ error: instance ? `Instancia "${instance}" nao encontrada.` : "Nenhuma instancia configurada." }, 400);
        const provider = (await loadInstances()).find((i: any) => i.instance_id === targetInst)?.provider ?? "zapi";
        const { status, data } = await callWaProxy("create-group", { groupName: name, phones: participants, autoInvite: auto_invite, ...(description && { description }) }, targetInst);
        if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
        const r = data?.result ?? {};
        const groupId = r.phone ?? r.id ?? r.group?.id ?? null;
        if (groupId) {
          await supabase.from("chats").upsert({ instance_id: targetInst, chat_id: String(groupId), chat_name: name, is_group: true, ...(description && { description }), last_message_at: new Date().toISOString() }, { onConflict: "instance_id,chat_id" });
          // Z-API nao aceita descricao na criacao — aplica em seguida.
          if (description && provider === "zapi") await callWaProxy("update-group-description", { groupId: String(groupId), groupDescription: description }, targetInst);
        }
        return json({ ok: true, group_id: groupId, ...(r.invitationLink && { invite_link: r.invitationLink }), ...(Array.isArray(r.phonesNotAdded) && r.phonesNotAdded.length && { phones_not_added: r.phonesNotAdded }), instance: targetInst, ...(groupId ? {} : { warning: "Provider nao devolveu o id do grupo.", raw: r }) });
      }

      case "group_info": {
        const { group, instance, sync = true } = params;
        if (!group) return json({ error: "group e obrigatorio." }, 400);
        const g = await resolveGroup(group, instance);
        if (g.error) return json({ error: g.error }, 400);
        if (g.candidates) return json({ ok: true, ambiguous: true, candidates: g.candidates });
        const { status, data } = await callWaProxy("group-metadata", { groupId: g.chat_id }, g.instance_id);
        if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
        const meta = normalizeGroupMetadata(g.provider, data?.result);
        if (!meta) return json({ ok: false, error: "Provider devolveu metadata vazio.", raw: data?.result });
        if (sync) await syncGroupToDb(g.instance_id, g.chat_id, meta);
        return json({ ok: true, chat_id: g.chat_id, ...meta, instance: g.instance_id, synced: !!sync });
      }

      case "group_update": {
        const { group, name, description, photo_url, settings, ephemeral, instance, confirmed = false } = params;
        if (!group) return json({ error: "group e obrigatorio." }, 400);
        const changes: Record<string, unknown> = {
          ...(name !== undefined && { name }), ...(description !== undefined && { description }),
          ...(photo_url && { photo_url }), ...(settings && { settings }), ...(ephemeral !== undefined && { ephemeral }),
        };
        if (!Object.keys(changes).length) return json({ error: "Informe pelo menos um campo: name, description, photo_url, settings ou ephemeral." }, 400);
        if (ephemeral !== undefined && ![0, 86400, 604800, 7776000].includes(Number(ephemeral))) return json({ error: "ephemeral deve ser 0 (off), 86400 (24h), 604800 (7d) ou 7776000 (90d)." }, 400);
        const g = await resolveGroup(group, instance);
        if (g.error) return json({ error: g.error }, 400);
        if (g.candidates) return json({ ok: true, ambiguous: true, candidates: g.candidates });
        if (!confirmed) return json({ blocked: true, needs_confirmation: true, group: g.chat_name ?? g.chat_id, changes, instruction: "Mostre o grupo e as mudancas ao usuario e reenvie com confirmed:true apos ele confirmar." });
        const applied: string[] = [], failed: any[] = [];
        const run = async (field: string, waAction: string, wparams: Record<string, unknown>) => {
          const { status, data } = await callWaProxy(waAction, wparams, g.instance_id);
          if (status >= 400 || data?.error) failed.push({ field, error: data?.error || `wa-proxy ${status}` });
          else applied.push(field);
        };
        if (name !== undefined) await run("name", "update-group-name", { groupId: g.chat_id, groupName: name });
        if (description !== undefined) await run("description", "update-group-description", { groupId: g.chat_id, groupDescription: description });
        if (photo_url) await run("photo_url", "update-group-photo", { groupId: g.chat_id, groupPhoto: photo_url });
        if (settings) {
          if (g.provider === "zapi") {
            // Z-API exige os 4 flags juntos: mergeia com os atuais antes de mandar.
            // Se a leitura falhar, ABORTA o campo — mandar cur={} zeraria em silêncio
            // os flags que o usuario NAO pediu pra mudar.
            const { status: mdStatus, data: md } = await callWaProxy("group-metadata", { groupId: g.chat_id }, g.instance_id);
            const cur = md?.result;
            if (mdStatus >= 400 || md?.error || !cur || typeof cur !== "object") {
              failed.push({ field: "settings", error: `nao deu pra ler os settings atuais do grupo (merge obrigatorio dos 4 flags Z-API): ${md?.error || `wa-proxy ${mdStatus}`}. Tente de novo.` });
            } else {
              await run("settings", "update-group-settings", {
                phone: g.chat_id,
                adminOnlyMessage: settings.admin_only_message ?? !!cur.adminOnlyMessage,
                adminOnlySettings: settings.admin_only_settings ?? !!cur.adminOnlySettings,
                requireAdminApproval: settings.require_admin_approval ?? !!cur.requireAdminApproval,
                adminOnlyAddMember: settings.admin_only_add_member ?? !!cur.adminOnlyAddMember,
              });
            }
          } else {
            if (settings.require_admin_approval !== undefined || settings.admin_only_add_member !== undefined)
              failed.push({ field: "settings", error: "require_admin_approval/admin_only_add_member nao existem no provider evolution (so Z-API)." });
            if (settings.admin_only_message !== undefined) await run("settings.admin_only_message", "update-group-settings", { groupId: g.chat_id, adminOnlyMessage: settings.admin_only_message });
            if (settings.admin_only_settings !== undefined) await run("settings.admin_only_settings", "update-group-settings", { groupId: g.chat_id, adminOnlySettings: settings.admin_only_settings });
          }
        }
        if (ephemeral !== undefined) {
          if (g.provider === "zapi") failed.push({ field: "ephemeral", error: "Mensagens temporarias nao existem no provider zapi (so Evolution)." });
          else await run("ephemeral", "toggle-ephemeral", { groupId: g.chat_id, expiration: Number(ephemeral) });
        }
        // Espelho local best-effort do que aplicou.
        if ((applied.includes("name") || applied.includes("description")) && g.instance_id) {
          await supabase.from("chats").update({
            ...(applied.includes("name") && { chat_name: name }),
            ...(applied.includes("description") && { description }),
          }).eq("chat_id", g.chat_id).eq("instance_id", g.instance_id);
        }
        return json({ ok: failed.length === 0, group: g.chat_name ?? g.chat_id, applied, ...(failed.length && { failed }) });
      }

      case "group_members": {
        const { group, action: gmAction, phones = [], instance, confirmed = false } = params;
        const GM_MAP: Record<string, string> = { add: "add-participant", remove: "remove-participant", promote: "add-admin", demote: "remove-admin", approve: "approve-participant", reject: "reject-participant" };
        const waAction = GM_MAP[gmAction];
        if (!waAction) return json({ error: `action invalida "${gmAction}". Use add|remove|promote|demote|approve|reject.` }, 400);
        if (!group || !Array.isArray(phones) || !phones.length) return json({ error: "group e phones (min 1) sao obrigatorios." }, 400);
        const g = await resolveGroup(group, instance);
        if (g.error) return json({ error: g.error }, 400);
        if (g.candidates) return json({ ok: true, ambiguous: true, candidates: g.candidates });
        if (g.provider === "evolution" && (gmAction === "approve" || gmAction === "reject"))
          return json({ error: "approve/reject de entrada pendente so existe no provider zapi." }, 400);
        if (!confirmed) return json({ blocked: true, needs_confirmation: true, group: g.chat_name ?? g.chat_id, action: gmAction, phones, instruction: "Mostre grupo + acao + telefones ao usuario e reenvie com confirmed:true apos ele confirmar." });
        const { status, data } = await callWaProxy(waAction, { groupId: g.chat_id, phones, ...(gmAction === "add" && { autoInvite: true }) }, g.instance_id);
        if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
        // Espelho local best-effort (o webhook de group_participant corrige depois se divergir).
        if (g.instance_id) {
          const digits = phones.map((p: any) => String(p).replace(/\D/g, "")).filter(Boolean);
          if (digits.length) {
            if (gmAction === "add" || gmAction === "approve") {
              await supabase.from("group_participants").upsert(digits.map((p: string) => ({ instance_id: g.instance_id, chat_id: g.chat_id, phone: p, joined_at: new Date().toISOString(), left_at: null })), { onConflict: "instance_id,chat_id,phone" });
            } else if (gmAction === "remove" || gmAction === "reject") {
              await supabase.from("group_participants").update({ left_at: new Date().toISOString() }).eq("instance_id", g.instance_id).eq("chat_id", g.chat_id).in("phone", digits);
            } else {
              // demote também limpa is_super_admin (superadmin rebaixado não pode ficar flagado)
              const patch: Record<string, boolean> = { is_admin: gmAction === "promote" };
              if (gmAction === "demote") patch.is_super_admin = false;
              await supabase.from("group_participants").update(patch).eq("instance_id", g.instance_id).eq("chat_id", g.chat_id).in("phone", digits);
            }
          }
        }
        return json({ ok: true, action: gmAction, group: g.chat_name ?? g.chat_id, phones, result: data?.result });
      }

      case "group_invite": {
        const { action: giAction = "get", group, url, instance, confirmed = false } = params;
        if (giAction === "info" || giAction === "accept") {
          if (!url) return json({ error: `url e obrigatoria para action=${giAction}.` }, 400);
          const targetInst = await defaultInstanceId(instance);
          if (!targetInst) return json({ error: instance ? `Instancia "${instance}" nao encontrada.` : "Nenhuma instancia configurada." }, 400);
          if (giAction === "info") {
            const { status, data } = await callWaProxy("group-invitation-metadata", { url }, targetInst);
            if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
            const provider = (await loadInstances()).find((i: any) => i.instance_id === targetInst)?.provider ?? "zapi";
            const meta = normalizeGroupMetadata(provider, data?.result);
            return json({ ok: true, ...(meta ?? { raw: data?.result }) });
          }
          if (!confirmed) return json({ blocked: true, needs_confirmation: true, url, instruction: "accept ENTRA no grupo com o numero da instancia. Confirme com o usuario e reenvie com confirmed:true." });
          const { status, data } = await callWaProxy("accept-invite", { url }, targetInst);
          if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
          return json({ ok: true, joined: true, result: data?.result });
        }
        if (giAction !== "get" && giAction !== "revoke") return json({ error: `action invalida "${giAction}". Use get|revoke|info|accept.` }, 400);
        if (!group) return json({ error: `group e obrigatorio para action=${giAction}.` }, 400);
        const g = await resolveGroup(group, instance);
        if (g.error) return json({ error: g.error }, 400);
        if (g.candidates) return json({ ok: true, ambiguous: true, candidates: g.candidates });
        if (giAction === "revoke" && !confirmed) return json({ blocked: true, needs_confirmation: true, group: g.chat_name ?? g.chat_id, instruction: "revoke INVALIDA o link de convite atual (quem tem o link antigo perde acesso). Confirme com o usuario e reenvie com confirmed:true." });
        const { status, data } = await callWaProxy(giAction === "revoke" ? "redefine-invitation-link" : "group-invitation-link", { groupId: g.chat_id }, g.instance_id);
        if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
        const r = data?.result ?? {};
        const link = r.invitationLink ?? r.inviteUrl ?? (r.inviteCode ? `https://chat.whatsapp.com/${r.inviteCode}` : null);
        return json({ ok: true, action: giAction, group: g.chat_name ?? g.chat_id, invite_link: link, ...(link ? {} : { raw: r }) });
      }

      case "group_leave": {
        const { group, instance, confirmed = false } = params;
        if (!group) return json({ error: "group e obrigatorio." }, 400);
        const g = await resolveGroup(group, instance);
        if (g.error) return json({ error: g.error }, 400);
        if (g.candidates) return json({ ok: true, ambiguous: true, candidates: g.candidates });
        if (!confirmed) return json({ blocked: true, needs_confirmation: true, group: g.chat_name ?? g.chat_id, ...(g.member_count != null && { member_count: g.member_count }), instruction: "Sair do grupo remove o numero da instancia dele (readmissao so por convite/admin). Confirme com o usuario e reenvie com confirmed:true." });
        const { status, data } = await callWaProxy("leave-group", { groupId: g.chat_id }, g.instance_id);
        if (status >= 400) return json({ ok: false, error: data?.error || `wa-proxy ${status}`, detail: data }, status);
        return json({ ok: true, left: true, group: g.chat_name ?? g.chat_id });
      }
...
const SERVER_INFO = { name: "whatsapp-agent", version: "3.5.0" };
...
const TOOL_SCHEMAS = [
  ...
  {
    name: "group_create",
    description: "Cria um grupo de WhatsApp com os participantes informados. FLUXO OBRIGATORIO: 1a chamada SEM confirmed (bloqueia e ecoa nome+participantes); 2a com confirmed:true apos o usuario confirmar. Retorna group_id e invite_link (quando o provider devolve).",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Nome do grupo" },
        participants: { type: "array", items: { type: "string" }, description: "Telefones dos participantes (formato internacional, sem +). Min 1; nao inclua o proprio numero." },
        description: { type: "string", description: "Descricao do grupo (aplicada apos a criacao no Z-API)" },
        auto_invite: { type: "boolean", description: "Envia convite no privado pra quem nao puder ser adicionado direto (default true)" },
        confirmed: { type: "boolean", description: "OBRIGATORIO true para criar; so apos o usuario confirmar" },
        instance: { type: "string", description: "De qual numero criar (alias ou instance_id)" },
      },
  ...
  {
    name: "group_info",
    description: "Metadata completo de um grupo: nome, descricao, dono, link de convite (quando disponivel), configuracoes e lista de participantes com admins. Por padrao sincroniza o resultado em chats/group_participants (sync:false desliga). 'group' aceita nome do grupo ou chat_id.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string", description: "Nome do grupo ou chat_id ('...@g.us' / '...-group')" },
        sync: { type: "boolean", description: "Espelha metadata no banco (default true)" },
        instance: { type: "string", description: "Instancia (alias ou instance_id)" },
      },
      required: ["group"],
      additionalProperties: false,
  ...
  {
    name: "group_update",
    description: "Atualiza dados/configuracoes de um grupo: nome, descricao, foto, settings (so admins mandam mensagem / editam dados; +2 flags so Z-API) e mensagens temporarias (ephemeral, so Evolution). FLUXO OBRIGATORIO: 1a chamada SEM confirmed (bloqueia e ecoa o diff); 2a com confirmed:true. Aplica cada campo em sequencia e reporta applied/failed.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string", description: "Nome do grupo ou chat_id" },
        name: { type: "string", description: "Novo nome do grupo" },
        description: { type: "string", description: "Nova descricao" },
        photo_url: { type: "string", description: "URL (ou base64) da nova foto do grupo" },
        settings: {
          type: "object",
          description: "Configuracoes do grupo (exige admin). require_admin_approval e admin_only_add_member so existem no Z-API.",
          properties: {
            admin_only_message: { type: "boolean", description: "So admins enviam mensagem" },
            admin_only_settings: { type: "boolean", description: "So admins editam dados do grupo" },
            require_admin_approval: { type: "boolean", description: "Entrada exige aprovacao de admin (so Z-API)" },
            admin_only_add_member: { type: "boolean", description: "So admins adicionam membros (so Z-API)" },
          },
          additionalProperties: false,
        },
        ephemeral: { type: "number", enum: [0, 86400, 604800, 7776000], description: "Mensagens temporarias em segundos: 0=off, 86400=24h, 604800=7d, 7776000=90d (so Evolution)" },
        confirmed: { type: "boolean", description: "OBRIGATORIO true para aplicar; so apos o usuario confirmar" },
        instance: { type: "string", description: "Instancia (alias ou instance_id)" },
      },
      required: ["group"],
      additionalProperties: false,
  ...
  {
    name: "group_members",
    description: "Gerencia participantes de um grupo: add (adiciona; convite no privado se nao der direto), remove, promote/demote (admin) e approve/reject (fila de entrada pendente, so Z-API). FLUXO OBRIGATORIO: 1a chamada SEM confirmed (bloqueia e ecoa grupo+acao+telefones); 2a com confirmed:true.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string", description: "Nome do grupo ou chat_id" },
        action: { type: "string", enum: ["add", "remove", "promote", "demote", "approve", "reject"], description: "O que fazer com os telefones" },
        phones: { type: "array", items: { type: "string" }, description: "Telefones alvo (formato internacional, sem +)" },
        confirmed: { type: "boolean", description: "OBRIGATORIO true para executar; so apos o usuario confirmar" },
        instance: { type: "string", description: "Instancia (alias ou instance_id)" },
      },
      required: ["group", "action", "phones"],
      additionalProperties: false,
    },
  },
  {
    name: "group_invite",
    description: "Link de convite de grupo: get (obtem o link atual), revoke (invalida e gera novo — exige confirmed), info (metadata de um grupo via link, sem entrar) e accept (ENTRA no grupo via link com o numero da instancia — exige confirmed). get/revoke usam 'group'; info/accept usam 'url'.",
    inputSchema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["get", "revoke", "info", "accept"], description: "Default get" },
        group: { type: "string", description: "Nome do grupo ou chat_id (para get/revoke)" },
        url: { type: "string", description: "Link de convite https://chat.whatsapp.com/... (para info/accept)" },
        confirmed: { type: "boolean", description: "Obrigatorio true para revoke/accept; so apos o usuario confirmar" },
        instance: { type: "string", description: "Instancia (alias ou instance_id)" },
      },
      additionalProperties: false,
    },
  ...
  {
    name: "group_leave",
    description: "Sai de um grupo com o numero da instancia (readmissao so por convite/admin). FLUXO OBRIGATORIO: 1a chamada SEM confirmed (bloqueia e mostra o grupo); 2a com confirmed:true apos o usuario confirmar.",
    inputSchema: {
      type: "object",
      properties: {
        group: { type: "string", description: "Nome do grupo ou chat_id" },
        confirmed: { type: "boolean", description: "OBRIGATORIO true para sair; so apos o usuario confirmar" },
        instance: { type: "string", description: "Instancia (alias ou instance_id)" },
      },
      required: ["group"],
      additionalProperties: false,
